...
//        }
    ...
    public int calculateFishQuality(FishingRodType fishingRod) {
        ...
        return (int) ((new Random().nextInt(2)) * (App.dataManager.getSkillLevels().get(Skill.FISHING).getLevel() + 2) * fishingRod.getPoleCoefficient() / (7 - App.dataManager.getCurrentGame().getWeather().getWeatherCoEfficient()));
