...
//        }
    ...
    public Result meetNPC(String npcName) {
        ...
        Situation currentSituation = new Situation( App.dataManager.getCurrentGame().getTime().getHour(),

                                                    App.dataManager.getCurrentGame().getTime().getSeason(),

                                                    App.dataManager.getCurrentGame().getWeather() );

        for ( String sentence : npc.getDialog().keySet() ) {
            if ( npc.getDialog().get(sentence).equals(currentSituation) ) {
                return new Result(true, sentence);
            }
        }

        return new Result(false, npcName + " doesn't have anything to say right now.");
    }
