...
//        setFriendship(App.dataManager.getCurrentGame());
    ...
    void testCheatTimeCommand() {
        ...
        Result result = gameController.cheatAdvanceTime("adwdwd2");
        assertFalse(result.success());
        assertEquals("Invalid time format",result.message());

        result = gameController.cheatAdvanceTime("2");
        assertTrue(result.success());
        assertEquals("New " + "Date is: " + game.getTime().getYear() + "." +
                (game.getTime().getMonth().getMonthIndex()+1) + "." +
                game.getTime().getDate() + " And " +"Time is: " + game.getTime().getHour(),result.message());
    ...
    void testCheatDateCommand() {
        ...
        Result result = gameController.cheatAdvanceDate("adwdwd2");
        assertFalse(result.success());
        assertEquals("Invalid date format",result.message());

        result = gameController.cheatAdvanceDate("2");
        assertTrue(result.success());
        assertEquals("New " + "Date is: " + game.getTime().getYear() + "." +
                (game.getTime().getMonth().getMonthIndex()+1) + "." +
                game.getTime().getDate() + " And " +"Time is: " + game.getTime().getHour(),result.message());
